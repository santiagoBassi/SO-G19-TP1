...
int execute_jobs_on_files(slave_worker* slave_workers, int num_slaves, char* files[], int num_file_args) {
    ...
    while (read_file_index < num_file_args) {
        ...
        for (int j = 0; j < num_slaves; j++) {
            max_fd = (slave_workers[j].pipes.in[W_END] > max_fd) ? slave_workers[j].pipes.in[W_END] : max_fd;
            max_fd = (slave_workers[j].pipes.out[R_END] > max_fd) ? slave_workers[j].pipes.out[R_END] : max_fd;

            FD_SET(slave_workers[j].pipes.in[W_END], &to_write_fds);
            FD_SET(slave_workers[j].pipes.out[R_END], &to_read_fds);
        }

        select(max_fd + 1, &to_read_fds, &to_write_fds, NULL, &timeout); 

        for (int j = 0; j < num_slaves; j++) {
            if (write_file_index < num_file_args && slave_workers[j].finished_job && FD_ISSET(slave_workers[j].pipes.in[W_END], &to_write_fds)) {
                write(slave_workers[j].pipes.in[W_END], files[write_file_index], strlen(files[write_file_index]));
                write(slave_workers[j].pipes.in[W_END], "\n", 1);

                slave_workers[j].finished_job = 0;

                write_file_index++;
            }

            if (FD_ISSET(slave_workers[j].pipes.out[R_END], &to_read_fds)) {
                char buff[SLAVE_OUTPUT_MAX_LEN];
                int chars_read = read(slave_workers[j].pipes.out[R_END], buff, SLAVE_OUTPUT_MAX_LEN);
                buff[chars_read] = '\0';

                printf("%d: %s\n", slave_workers[j].pid, buff);

                slave_workers[j].finished_job = 1;
